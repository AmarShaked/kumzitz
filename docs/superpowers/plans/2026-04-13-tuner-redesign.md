# Tuner Page Redesign Implementation Plan

> **For agentic workers:** REQUIRED SUB-SKILL: Use superpowers:subagent-driven-development (recommended) or superpowers:executing-plans to implement this plan task-by-task. Steps use checkbox (`- [ ]`) syntax for tracking.

**Goal:** Replace the needle-gauge tuner UI with a GuitarTuna-inspired layout: horizontal tuning graph with a moving cents bubble, strings flanking a minimal SVG headstock, instrument cycling via top bar tap.

**Architecture:** Single file rewrite of `src/features/tuner/TunerPage.tsx`. The `useTuner` hook and all instrument configs are preserved. Four internal components: `HeadstockSVG`, `TuningGraph`, `StringCircle`, and the default `TunerPage` export.

**Tech Stack:** React, TypeScript, Tailwind CSS, lucide-react

---

### Task 1: Rewrite `TunerPage.tsx`
...
Write the following as the complete contents of `src/features/tuner/TunerPage.tsx`:
...
function TuningGraph({
  ...
  // maps -50..+50 cents to -45%..+45% offset from center
  ...
  // label sits on the inner side of the bubble (toward center)
...
Expected: all tests pass (the tuner hook tests are unchanged).

- [ ] **Step 4: Commit**

```bash
git add src/features/tuner/TunerPage.tsx
git commit -m "feat: redesign tuner page with GuitarTuna-inspired layout"
```
